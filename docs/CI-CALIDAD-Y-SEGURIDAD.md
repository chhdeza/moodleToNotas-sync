...
- **OSV-Scanner se llama por su acción directa, no por el flujo reutilizable oficial de Google.** Ese flujo reutilizable pide permiso `security-events: write` (para subir el resultado a la pestaña Security) sin importar la configuración, y un flujo reutilizable no puede pedir más permisos de los que le dio quien lo llamó. Como este flujo solo pide `contents: read` en toda la cadena, llamar a su flujo reutilizable hacía fallar el arranque completo con "workflow is not valid" — no un hallazgo, ni un job en rojo: el flujo entero rehusaba correr. La acción directa se queda con eso mismo fijado por hash y sin pedir permisos de más; a cambio, no hay subida a *Security → Code Scanning*, solo el resultado en el log del job.
